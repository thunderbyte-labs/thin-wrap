        ...
        try:
            ...
            if "://" in url:
                ...
                if protocol.lower() in ["socks5", "socks4"]:
                    self.proxy_type = (
                        socks.SOCKS5 if protocol.lower() == "socks5" else socks.SOCKS4
                    )
            ...
            logger.info(
                f"Parsed proxy: {self.proxy_host}:{self.proxy_port} (type: {self.proxy_type})"
            )
            return True
...
class SOCKSProxyWrapper(ProxyWrapper):
    ...
    def _setup_proxy_config(self):
        ...
        try:
            ...
            logger.debug(
                f"Proxy configured: {self.proxy_config.proxy_host}:{self.proxy_config.proxy_port} (type: {self.proxy_config.proxy_type})"
            )
            return True
        except Exception as e:
    ...
    def get_connection_info(self):
        if self.proxy_config:
            proxy_type_str = (
                "HTTP"
                if self.proxy_config.proxy_type == "http"
                else (
                    "SOCKS5"
                    if self.proxy_config.proxy_type == socks.SOCKS5
                    else "SOCKS4"
                )
            )
            return {
